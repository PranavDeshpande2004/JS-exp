...
function compareStrings() {
  const string1 = document.getElementById("string1").value.trim();
  const string2 = document.getElementById("string2").value.trim();
  const resultDiv = document.getElementById("comparisonResults");

  resultDiv.innerHTML = "";

  if (!string1 || !string2) {
    resultDiv.innerHTML = "<p style='color:red;'>⚠️ Please enter both strings to compare.</p>";
    return;
  }
  ...
  // Length comparison
  let lengthCompareResult = "Equal length";
  ...
  // Alphabetical comparison using localeCompare
